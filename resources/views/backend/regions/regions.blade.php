...
@section('subcontent')
    ...
    <div class="intro-y mt-5 flex flex-col items-center sm:flex-row">
        ...
        <div class="mt-4 flex w-full sm:mt-0 sm:w-auto">
            <a href="{{ route('regions_add') }}"
                class="transition duration-200 border inline-flex items-center justify-center py-2 px-3 rounded-md font-medium cursor-pointer focus:ring-4 focus:ring-primary focus:ring-opacity-20 focus-visible:outline-none dark:focus:ring-slate-700 dark:focus:ring-opacity-50 [&amp;:hover:not(:disabled)]:bg-opacity-90 [&amp;:hover:not(:disabled)]:border-opacity-90 [&amp;:not(button)]:text-center disabled:opacity-70 disabled:cursor-not-allowed bg-primary border-primary text-white dark:border-primary mr-2 shadow-md">เพิ่มทวีป</a>
        </div>
    </div>
    ...
    <div class="lg:flex intro-y mt-5 mb-5">
        ...
        <div class="relative">
            <input type="text" name="keyword" id="keyword" class="form-control py-3 px-4 w-full lg:w-64 box pr-10"
                placeholder="ชื่อทวีป..." value="{{ request()->input('keyword') }}" onkeypress="handleEnter(event)">
                        ...
                        <tr class="intro-x">
                            ...
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    <a class="flex items-center" href="{{ route('regions_edit', ['id' => $res->id]) }}">
...
@section('script')
    <script>
        function applyFilters() {
            ...
            var newUrl = `{{ route('regions') }}?keyword=${keyword}`;
