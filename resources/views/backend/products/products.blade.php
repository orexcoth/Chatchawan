...
@section('subcontent')
    ...
    <div class="intro-y mt-5 flex flex-col items-center sm:flex-row">
        ...
        <div class="mt-4 flex w-full sm:mt-0 sm:w-auto">
            <a href="{{ route('products_add') }}" class="btn btn-primary shadow-md mr-2">เพิ่ม{{ $default_pagename }}</a>
        </div>
    </div>
    ...
    <div class="lg:flex intro-y mt-5 mb-5">
        ...
        <div class="relative">
            <input type="text" name="keyword" id="keyword" class="form-control py-3 px-4 w-full lg:w-64 box pr-10"
                placeholder="ชื่อสินค้า..." value="{{ request()->input('keyword') }}" onkeypress="handleEnter(event)">
            ...
            <table class="table table-report -mt-2">
                <thead>
                    <tr>
                        ...
                        <th class="whitespace-nowrap">ขนาด</th>
                        <th class="whitespace-nowrap">หน่วย</th>
                        {{-- <th class="whitespace-nowrap">ประเภทกล่องที่ใช้</th> --}}
                ...
                <tbody>
                    ...
                    @foreach ($query as $keyres => $res)
                        ...
                        <tr class="intro-x">
                            ...
                            <td>
                                <div class="font-medium whitespace-nowrap">{{ $res->product_code }}</div>
                            </td>
                            ...
                            <td>
                                <div class="font-medium whitespace-nowrap" style="float: right;
                                ">{{ number_format($res->weight,2) }}</div>
                            </td>
                            <td>
                                <div class="font-medium whitespace-nowrap">{{ $res->unit->name }}</div>
                            </td>
                            {{-- <td>
                                <div class="font-medium whitespace-nowrap">{{ $res->unit->name }}</div>
                            </td> --}}
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    ...
                                    <a class="flex items-center" href="{{ route('products_edit', ['id' => $res->id]) }}">
...
@section('script')
    <script>
        function applyFilters() {
            ...
            var newUrl = `{{ route('products') }}?keyword=${keyword}`;
